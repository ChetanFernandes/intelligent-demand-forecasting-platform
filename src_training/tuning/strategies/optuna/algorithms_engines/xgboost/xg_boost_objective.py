from src_training.tuning.strategies.optuna.algorithms_engines.xgboost.xg_boost_training import XGBoostTrainingEngine
...
class XGBoostObjective:
    ...
    def __call__(self, trial): # Python allows objects with __call__ to behave like functions. By Python converts it to obj.__call__(). 
        ...
        engine = XGBoostTrainingEngine(estimator=self.estimator, X_train=self.X_train, y_train=self.y_train, n_splits=self.n_splits)
